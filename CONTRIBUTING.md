...
The v0.3.2 release contract defines macOS 26 and Ubuntu 24.04 LTS as the
...
- Update `docs/architecture.md` when a change alters observable behavior or a
  system boundary.
- For every public process-isolation claim, link the v0.3.2 native evidence
...
### v0.3.2 evidence and documentation contract

The v0.3.2 public protection contract is tied to the credential-free native
...
identities before pushing only the printed tag ref. For v0.3.2, that command is:
...
require exact `acs v0.3.2` output. Independently verify the public archive with
...
GOBIN="$tagged_gobin" go install github.com/alcimerio/ai-config-selector/cmd/acs@v0.3.2
"$tagged_gobin/acs" version
rm "$tagged_gobin/acs"
rmdir "$tagged_gobin"
```

The tagged installation must print `acs v0.3.2`. A local checkout without
