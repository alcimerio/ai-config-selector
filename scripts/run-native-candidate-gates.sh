...
ACS_RUN_MCP_AMBIENT_FEASIBILITY=1 ACS_TEST_DEVIN_BINARY="$devin_binary" go test ./internal/launch -run '^TestSeatbeltCandidate(MCPAmbientReadDenialWithAbsentAtPrepareAndAliases|MCPRecipeWriteAndAncestorDenialsPreserveOrdinaryHome|PinnedDevinUsesSelectedHomeMCPConfigOnly|PinnedDevinConfigPathReplacementIsolation|PinnedDevinNestedDiscoveryGrantScope|PinnedDevinDirectorySymlinkRedirection)$' -count=1 -v
