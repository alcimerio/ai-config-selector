...
func TestPrepareReleaseTagCreatesValidatedLocalTagWithoutPushing(t *testing.T) {
	repository, remote := prepareReleaseTagRepository(t)
	command := exec.Command("sh", "scripts/prepare-release-tag.sh", "v1.2.3")
	...
	if got := gitOutput(t, repository, "for-each-ref", "--format=%(contents)", "refs/tags/v1.2.3"); strings.TrimSpace(got) != "Release v1.2.3" {
...
func TestPrepareReleaseTagRejectsSourceThatDoesNotMatchOriginMain(t *testing.T) {
	repository, _ := prepareReleaseTagRepository(t)
...
func prepareReleaseTagRepository(t *testing.T) (string, string) {
	t.Helper()
