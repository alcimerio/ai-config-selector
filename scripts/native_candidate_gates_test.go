...
func TestNativeCandidateGatesPropagateFailureRecoverAndProtectIdentity(t *testing.T) {
	...
	t.Run("all gates and recovery succeed", func(t *testing.T) {
		...
		for _, required := range []string{
			...
			"go test ./internal/launch -run ^TestSeatbeltCandidate(MCPAmbientReadDenialWithAbsentAtPrepareAndAliases|MCPRecipeWriteAndAncestorDenialsPreserveOrdinaryHome|PinnedDevinUsesSelectedHomeMCPConfigOnly|PinnedDevinConfigPathReplacementIsolation|PinnedDevinNestedDiscoveryGrantScope|PinnedDevinDirectorySymlinkRedirection)$ -count=1 -v",
