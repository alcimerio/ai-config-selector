...
func TestContextualHelpWithoutRuntime(t *testing.T) {
	for _, command := range []string{"", "profile", "profile list", "profile show", "run", "devin", "devin create-profile", "sandbox", "codex", "codex create-profile", "codex auth", "codex auth login", "codex auth list", "codex auth status", "codex auth recover", "codex auth logout", "version"} {
