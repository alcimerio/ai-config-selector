...
func TestLaunchDelegatesResolvedProfileAndTerminalToLauncher(t *testing.T) {
	...
	if exitCode := application.Run(context.Background(), []string{"devin", "--profile", "reviews"}); exitCode != 23 {
		t.Fatalf("exit code = %d, want launcher exit code 23", exitCode)
	}
	if launcher.calls != 1 {
	...
	if launcher.sessionsDirectory != sessionsDirectory || launcher.workingDirectory != workingDirectory {
		t.Fatalf("launcher paths = (%q, %q), want (%q, %q)", launcher.sessionsDirectory, launcher.workingDirectory, sessionsDirectory, workingDirectory)
	}
	if _, err := launcher.resolved.Plan(context.Background(), workingDirectory); err != nil {
		t.Fatalf("launcher received unusable resolved Profile: %v", err)
	}
...
func TestLaunchLoadsVersionOneProfileWithoutRewritingIt(t *testing.T) {
	...
	application := cli.App{
		Categories: adapter.Categories(), Launcher: launcher,
		Profiles:          profile.NewStore(acsHome, adapter.Categories()),
		SessionsDirectory: filepath.Join(acsHome, "sessions"),
		WorkingDirectory:  t.TempDir(), Input: strings.NewReader(""),
...
type recordingProfileLauncher struct {
	calls             int
	exitCode          int
	err               error
...
func (launcher *recordingProfileLauncher) Launch(
	...
	launcher.calls++
	launcher.sessionsDirectory = sessionsDirectory
	launcher.workingDirectory = workingDirectory
	launcher.resolved = resolved
