...
func TestProductionRegistryRecoveryWithoutMarkerIsIdempotent(t *testing.T) {
	...
	registry, err := New(Config{BinaryPath: "/usr/bin/true", ACSHome: filepath.Join(root, "acs"), SessionsDirectory: sessions, WorkingDirectory: root})
	if err != nil {
		t.Fatal(err)
	}
...
func TestResourcePendingTransferWaitsForLateCleanupNotification(t *testing.T) {
	...
	(&Registry{}).transferResourcePendingBinding(created, binding, "challenge", lateCleanupProcess{done: cleanupDone})
	select {
	case <-settled:
...
func TestRegistryLoginUnsupportedAuthRemovesProjectionAndDoesNotCreate(t *testing.T) {
	...
	if _, err := registry.Login(context.Background(), LoginRequest{Name: "work"}); err != ErrUnsupportedAuth {
...
func TestRegistryLoginContainedFailureRemovesProjectionAndDoesNotCreate(t *testing.T) {
	...
	if _, err := registry.Login(context.Background(), LoginRequest{Name: "work"}); err != ErrLoginFailed {
